...
class Admin extends CI_Controller {
    ...
    function __construct() {
        parent::__construct();
        $this->load->model('getklasifikasi_model');
        $this->load->model('getskpd_model');
    }
    ...
    public function block_category() {
        ...
        if ($mau_ke == "cari") {
            ...
            */
            ...
            $a['page'] = "form_block_cat_add";
            $a['head'] = "head/head_dashboard";
            $a['footer'] = "footer/footer_umum";
            $this->load->view('admin/aaa', $a);
        } else if ($mau_ke == "edit") {
            $a['page'] = "form_block_cat_edt";
            $a['head'] = "head/head_dashboard";
            $a['footer'] = "footer/footer_umum";
            $this->load->view('admin/aaa', $a);
    ...
    public function surat_masuk() {
        ...
        /* pagination */
        $total_row = $this->db->query("SELECT * FROM surat_masuk_view")->num_rows();
        $per_page = 10;
        ...
        $config['allowed_types'] = 'bmp|jpg|png|pdf|doc|docx';
        ...
        if ($mau_ke == "del") {
            ...
            $this->db->query("UPDATE t_surat_masuk SET flag_active = '0' WHERE id = '$idu' ");
            $this->session->set_flashdata("k", "<div class=\"alert alert-success\" id=\"alert\">Data has been deleted </div>");
            redirect('index.php/admin/surat_masuk');
        } else if ($mau_ke == "cari") {
            $a['data'] = $this->db->query("SELECT * FROM surat_masuk_view WHERE isi_ringkas LIKE '%$cari%' OR dari LIKE '%$cari%' OR no_surat LIKE '%$cari%' ORDER BY id DESC")->result();
            $a['page'] = "l_surat_masuk";
        } else if ($mau_ke == "add") {
            $a['klasifikasi'] = $this->getklasifikasi_model->get_klas_surat();
            $a['listskpd'] = $this->getskpd_model->get_list_skpd();
            $a['page'] = "f_surat_masuk";
        } else if ($mau_ke == "edt") {
            $a['datpil'] = $this->db->query("SELECT * FROM surat_masuk_view WHERE id = '$idu'")->row();
            $a['klasifikasi'] = $this->getklasifikasi_model->get_klas_surat();
            $a['listskpd'] = $this->getskpd_model->get_list_skpd();
            $a['page'] = "f_surat_masuk";
        } else if ($mau_ke == "act_add") {
            if ($kode === "0"){
                echo "<script language=\"javascript\">alert('Kode klasifikasi harus diisi!');</script>";
                echo "<script language=\"javascript\">history.back();</script>";
            } 
            else if ($dari === "0"){
                echo "<script language=\"javascript\">alert('Surat darimana? Tolong diisi!');</script>";
                echo "<script language=\"javascript\">history.back();</script>";
            }
            else {
                if ($this->upload->do_upload('file_surat')) {
                    $up_data = $this->upload->data();
                    $this->db->query("INSERT INTO t_surat_masuk VALUES (NULL, '$kode', '$no_agenda', '$uraian', '$dari', '$no_surat', '$tgl_surat', NOW(), '$ket', '" . $up_data['file_name'] . "', '" . $this->session->userdata('admin_id') . "', '1')");
                } else {
                    $this->db->query("INSERT INTO t_surat_masuk VALUES (NULL, '$kode', '$no_agenda', '$uraian', '$dari', '$no_surat', '$tgl_surat', NOW(), '$ket', '', '" . $this->session->userdata('admin_id') . "', '1')");
                }
            ...
            if ($kode === "0"){
                ...
                if ($this->upload->do_upload('file_surat')) {
                    $up_data = $this->upload->data();
                    $this->db->query("UPDATE t_surat_masuk SET kode = '$kode', no_agenda = '$no_agenda', isi_ringkas = '$uraian', dari = '$dari', no_surat = '$no_surat', tgl_surat = '$tgl_surat', keterangan = '$ket', file = '" . $up_data['file_name'] . "' WHERE id = '$idp'");
                } else {
                    $this->db->query("UPDATE t_surat_masuk SET kode = '$kode', no_agenda = '$no_agenda', isi_ringkas = '$uraian', dari = '$dari', no_surat = '$no_surat', tgl_surat = '$tgl_surat', keterangan = '$ket' WHERE id = '$idp'");
                }
                $this->session->set_flashdata("k", "<div class=\"alert alert-success\" id=\"alert\">Data has been updated. " . $this->upload->display_errors() . "</div>");
                redirect('index.php/admin/surat_masuk');
            }
        } else {
            $a['data'] = $this->db->query("SELECT * FROM surat_masuk_view LIMIT $awal, $akhir ")->result();
            $a['page'] = "l_surat_masuk";
        }
        $this->load->view('admin/aaa', $a);
    }
    ...
    public function surat_disposisi() {
        ...
        if ($mau_ke == "del") {
            ...
            $a['page'] = "f_surat_disposisi";
        } else if ($mau_ke == "edt_bag") {
            $a['datpil'] = $this->db->query("SELECT * FROM t_disposisi WHERE id_surat = '$idu1'")->row();
            $a['datdep'] = $this->db->query("SELECT * FROM t_department")->result();
            $a['page'] = "f_surat_disposisi_kabag";
        } else if ($mau_ke == "act_add") {
    ...
    public function manage_admin() {
        ...
        if ($mau_ke == "del") {
            ...
            $a['datdep'] = $this->db->query("SELECT * FROM t_department ")->result();
            $a['datrole'] = $this->db->query("SELECT * FROM t_role ")->result();
        } else if ($mau_ke == "edt") {
            $a['datpil'] = $this->db->query("SELECT t_admin.*,t_department.department as ndepartment FROM t_admin inner join t_department on t_department.id=t_admin.department  WHERE t_admin.id = '$idu'")->row();
            $a['datdep'] = $this->db->query("SELECT * FROM t_department ")->result();
            $a['datrole'] = $this->db->query("SELECT * FROM t_role ")->result();
            $a['page'] = "f_manage_admin";
        } else if ($mau_ke == "act_add") {
            $this->db->query("INSERT INTO t_admin VALUES (NULL, '$username', '$password', '$nama', '$nip', '$level','$department')");
            $this->session->set_flashdata("k", "<div class=\"alert alert-success\" id=\"alert\">Data has been added</div>");
            redirect('index.php/admin/manage_admin');
        } else if ($mau_ke == "act_edt") {
            if ($password = md5("-")) {
                $this->db->query("UPDATE t_admin SET username = '$username', nama = '$nama', nip = '$nip', level = '$level', department = '$department' WHERE id = '$idp'");
            } else {
                $this->db->query("UPDATE t_admin SET username = '$username', password = '$password', nama = '$nama', nip = '$nip', level = '$level', department = '$department' WHERE id = '$idp'");
            }
    ...
    public function get_klasifikasi() {
        ...
        //$this->load->view('admin/f_surat_masuk', $data);
    ...
    public function do_login() {
        ...
        if ($j_cek == 1) {
            ...
            $this->session->set_flashdata("k", "<div id=\"alert\" class=\"alert alert-error\">username atau password tidak valid!</div>");
            redirect('index.php/admin/login');
        }
    }
